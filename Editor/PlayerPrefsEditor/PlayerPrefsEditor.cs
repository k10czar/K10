...
public class PlayerPrefsEditor : EditorWindow
{
	...
	public bool IsWindows { get { return Application.platform == RuntimePlatform.WindowsEditor; } }
	[MenuItem("K10/PlayerPrefs Editor")] private static void Init() { GetWindow<PlayerPrefsEditor>( "PlayerPrefs" ); }
	...
	private void OnGUI()
	{
		...
		if( GUILayout.Button( "Save", EditorStyles.toolbarButton ) ) SaveAll();
		if( GUILayout.Button( "Restore", EditorStyles.toolbarButton ) ) RefreshPlayerPrefs();
		...
		} else
		{
			for( int i = 0; i < playerPrefs.Count; i++ )
			{
				...
				{
					case PrefType.Int:
						playerPrefs[ i ].value.intValue = EditorGUILayout.IntField( playerPrefs[ i ].name, playerPrefs[ i ].value.intValue, EditorStyles.textField );
					break;
					case PrefType.Float:
						playerPrefs[ i ].value.floatValue = EditorGUILayout.FloatField( playerPrefs[ i ].name, playerPrefs[ i ].value.floatValue, EditorStyles.textField );
					break;
					case PrefType.String:
						playerPrefs[ i ].value.stringValue = EditorGUILayout.TextField( playerPrefs[ i ].name, playerPrefs[ i ].value.stringValue, EditorStyles.textField );
					break;
				}
				//GUILayout.FlexibleSpace();
				playerPrefs[ i ].value.type = (PrefType)EditorGUILayout.EnumPopup( playerPrefs[ i ].value.type, GUILayout.MaxWidth( 80 ) );
	...
	private void RefreshPlayerPrefs()
	{
		...
		playerPrefs = new List<PlayerPrefStore>();
		if( IsWindows ) GetPrefKeysWindows();
		else GetPrefKeysMac();
	}
