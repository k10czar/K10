...
namespace K10
{
	public static class Physics2DUtils
	{
		...
		[MethodImpl( AggrInline )] public static float GetProjectedRadiusMaxSize( Camera cam, Vector3 worldPos, float radius )
		{
		...
		[MethodImpl( AggrInline )] public static float GetProjectedRadiusAverageSize( Camera cam, Vector3 worldPos, float radius )
		{
			float up, down, right, left;
			GetProjectedRadiusSizes( cam, worldPos, radius, out right, out up, out left, out down );
		...
		[MethodImpl( AggrInline )] public static float GetProjectedRadiusMinSize( Camera cam, Vector3 worldPos, float radius )
		{
			float up, down, right, left;
			GetProjectedRadiusSizes( cam, worldPos, radius, out right, out up, out left, out down );
		...
		[MethodImpl( AggrInline )] public static void GetProjectedRadiusSizes( Camera cam, Vector3 worldPos, float radius, out float right, out float up, out float left, out float down )
		{
		...
		[MethodImpl( AggrInline )] static Vector2 Test( Camera cam, Matrix4x4 mat, Vector3 pos )
		{
		...
		[MethodImpl( AggrInline )] public static bool Collinear( Vector2 p, Vector2 q, Vector2 r ) { return Mathf.Approximately( Cross( q - p, r - p ), 0 ); }
		[MethodImpl( AggrInline )] public static float Cross( Vector2 a, Vector2 b ) { return a.x * b.y - a.y * b.x; }
		[MethodImpl( AggrInline )] public static bool CCW( Vector2 p, Vector2 q, Vector2 r ) { return Cross( q - p, r - p ) > 0; }

		[MethodImpl( AggrInline )] public static float Angle( Vector2 a, Vector2 o, Vector2 b )
		{
			var oa = a - o;
			var ob = b - o;
		...
		[MethodImpl( AggrInline )] public static float DistanceToLine( Vector2 p, Ray2D ray, out Vector2 hitPoint )
		{
		...
		[MethodImpl( AggrInline )] public static float DistanceToLine( Vector2 p, Vector2 a, Vector2 b, out Vector2 hitPoint )
		{
		...
		[MethodImpl( AggrInline )] public static float DistanceToLine( Vector2 p, Vector2 a, Vector2 b, out Vector2 hitPoint, float projA, float projB )
		{
			var dx = b.x - a.x;
			var dy = b.y - a.y;
		...
		[MethodImpl( AggrInline )] public static float DistanceToPoint( Vector2 p, Vector2 rayOrigin, Vector2 rayDirection, out Vector2 hitPoint, float range = float.MaxValue )
		{
		...
		[MethodImpl( AggrInline )] public static float DistanceToPoint( Vector2 p, Vector2 a, Vector2 b, out Vector2 hitPoint )
		{
			var projA = ( b.x - a.x ) * ( p.x - a.x ) + ( b.y - a.y ) * ( p.y - a.y );
		...
		[MethodImpl( AggrInline )] public static bool AreParellel( Vector2 l1a, Vector2 l1b, Vector2 l2a, Vector2 l2b )
		{
		...
		[MethodImpl( AggrInline )] public static float FindScale( Vector2 l1a, Vector2 l1b )
		{
			if( Mathf.Approximately( l1a.x, 0 ) ) return l1b.y / l1a.y;
			return l1b.x / l1a.x;
		}

		[MethodImpl( AggrInline )] public static bool Intersects( Vector2 l1a, Vector2 l1b, Vector2 l2a, Vector2 l2b, out Vector2 hitPoint )
		{
			hitPoint = l1a;
		...
		[MethodImpl( AggrInline )] public static float DistanceToLine( Ray ray, Vector3 point, out Vector3 projectedPoint )
		{
		...
		[MethodImpl( AggrInline )] public static float DistanceToLine( Ray ray, Vector3 point )
		{
			return Vector3.Cross( ray.direction, point - ray.origin ).magnitude;
		}

		[MethodImpl( AggrInline )] public static bool Aprox( float a, float b ) => Mathf.Abs( a - b ) < 2 * float.Epsilon;


		[MethodImpl( AggrInline )] public static Vector3 PlaneClosestPoint( Vector3 position, Vector3 planeNormal, Vector3 planeOrigin )
		{
		...
		[MethodImpl( AggrInline )] public static bool NormalsAreParallel( Vector3 planeNormal, Vector3 originToPoint )
		{
